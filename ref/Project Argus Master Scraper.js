...
(function(){
  if (!window.__argusKV) {
    window.__argusKV = {
      get(k, d){ try { return typeof GM_getValue==='function' ? GM_getValue(k, d) : (JSON.parse(localStorage.getItem(k)) ?? d); } catch (e) { /* ignore */ return d; } },
      set(k, v){ try { return typeof GM_setValue==='function' ? GM_setValue(k, v) : localStorage.setItem(k, JSON.stringify(v)); } catch (e) { /* ignore */ } },
    };
  }
  ...
  if (typeof __argusUI.renderProgressFromKV !== 'function') {
    ...
    __argusUI.renderProgressFromKV = function(){
      ...
      (function step(ts){
        ...
        try { if (typeof window.renderProgress==='function') window.renderProgress(shown); } catch (e) { /* ignore */ }
...
const Storage = {
  ...
  async set(k, v) { try { await GM_setValue(k, v); } catch (e) { /* ignore */ } },
  async del(k) { try { await GM_deleteValue(k); } catch (e) { /* ignore */ } },
  async keys() { try { return await GM_listValues(); } catch (e) { return []; } },
};
...
`);
  ...
  function setStateLabel(){
    ...
    try { el.textContent = 'State: ' + (window.__argusExtract && window.__argusExtract.getState ? window.__argusExtract.getState() : 'N/A'); } catch (e) { el.textContent = 'State: N/A'; }
  }
  setStateLabel();
...
function normalizePlaceKey(href){
  ...
  } catch (e) { return String(href||'').trim(); }
}
...
function importSingleLinkPrompt(){
  ...
  try{ console.log('[Argus][Trace] import:single ok', { key: key, href: href }); }catch(e){ /* ignore */ }
  try{
...
function armLinksMiner(){
  ...
  return; }
    ...
    function safeClearInterval(ref){ if (ref) { try { clearInterval(ref); } catch (e) { /* ignore */ } } return null; }
            ...
            if (msg.type === 'worker_done') {
                try {
                    ...
                    if (state.activeWorkers && state.activeWorkers[msg.url]) {
                        try { state.activeWorkers[msg.url].tab.close(); } catch (e) { /* ignore */ }
        ...
        try {
            ...
            if (!mainPanel) {
                ...
                while (idle < MAX_IDLE) {
                    ...
                    try { document.querySelectorAll(CONFIG.scraper.moreButtonSelector).forEach(btn => btn.click()); } catch (e) { /* ignore */ }
...
// =================== [/Argus][HD] ==========================================================
            ...
            // Expand any remaining short reviews and extract
            try { document.querySelectorAll(CONFIG.scraper.moreButtonSelector).forEach(button => button.click()); } catch (e) { /* ignore */ }
            await sleep(1500);
            ...
            try {
                ...
                if (sc) panelCandidates.push(sc);
                                 document.querySelectorAll('div[role="dialog"] .m6QErb, div[role="dialog"] .section-scrollbox').forEach(e => { if (argusIsScrollableProbe(e)) panelCandidates.push(e); });
                 document.querySelectorAll('div.m6QErb.DxyBCb, div[role="region"].m6QErb, div.section-scrollbox').forEach(e => { if (argusIsScrollableProbe(e)) panelCandidates.push(e); });
            } catch (e) { /* ignore */ }
            ...
            while (idle < MAX_IDLE) {
                if (panel) {
                    ...
                    try { document.querySelectorAll(CONFIG.scraper.moreButtonSelector).forEach(b => b.click()); } catch (e) { /* ignore */ }
                } else {
