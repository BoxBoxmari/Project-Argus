...
function load(p:string){ return fs.existsSync(p)?JSON.parse(fs.readFileSync(p,'utf8')):null; }
...
function stablePassRate(days=3){
  ...
  const stable = Object.entries(hist).filter(([k])=>k.startsWith('SIM#')||k.startsWith('REAL#'))
    .filter(([_,arr]:any)=>arr.length>=cutoff);
  let pass=0,total=0;
  for (const [,arr] of stable as any){ total+=arr.length; pass+=arr.filter((s:string)=>s==='passed').length; }
...
function dupRateFromDatasets(){
  ...
  const total = rows.length; const uniq = new Set(rows.map((r:any)=>r.id)).size;
  return total? 1 - (uniq/total) : 0;
}

const pass3d = stablePassRate(3);
const open = Number(metricsReal.open||0); const pane = Number(metricsReal.pane||0);
const dupr = dupRateFromDatasets();

const report = [
  '# OPS REPORT',
  `pass_rate_3d: ${pct(pass3d)}%`,
  `p95_open_ms: ${open}`, `p95_pane_ms: ${pane}`,
  `dupRate: ${pct(dupr)}%`,
