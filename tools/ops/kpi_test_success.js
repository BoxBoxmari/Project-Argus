...
const datasetDir = 'apps/scraper-playwright/datasets/datasets/default'; // Use test data directory
...
function dupRateFromDatasets(){
  ...
  const files = fs.readdirSync(datasetDir).filter(f => f.endsWith('.json') && f !== 'data.json'); // Exclude the problematic data.json
