...
(async () => {
  ...
  const aborted = new Set(); // track requests we intentionally abort (images/fonts/css/media)
  try {
    ...
    page.on('requestfailed', (r) => {
      const u = r.url();
      if (aborted.has(u)) return; // do not count intentional aborts
      append('events.jsonl', { type:'requestfailed', url:u, method:r.method(), failure:r.failure()?.errorText });
    });
    ...
    // Nếu panel review đã có thì khỏi click, log 'tab_already_open'
    ...
    // Scroll reviews + click "More" (PASS args correctly)
    ...
    try {
      const lines = fs.existsSync(logFile('events.jsonl'))
        ? fs.readFileSync(logFile('events.jsonl'),'utf8').trim().split(/\r?\n/).filter(Boolean).map(l=>JSON.parse(l))
        : [];
      const summary = {
        url,
